...
namespace MediaControlApp.Tests.App.Commands.Medias
{
    public class SetMediaConsumedCommandTests
    {
        ...
        [Fact]
        public void SetMediaConsumedCommandTests_Execute_ReturnsInt()
        {
            string? guidString = "4765d892-4211-4e3c-a022-86249eae09cc";
            //Arrange
            A.CallTo(() => _mediaService.SetConsumed(A<Guid>._, A<DateTime>._)).Returns(true);
            ...
            {
                builder.AddCommand<SetMediaConsumedCommand>("media set-consumed");
            });
            app.SetDefaultCommand<SetMediaConsumedCommand>();

            // Act
            var result = app.Run(guidString, DateTime.Now.AddYears(-3).ToShortDateString());
        ...
        public void RemoveAuthorCommandTests_Execute_Interactive_ReturnsInt()
        {
            ...
            console.Input.PushTextWithEnter(DateTime.Now.ToShortDateString());
