...
namespace MediaControlApp.Infrastructure.DataAccess.MediaStore.Repositories
{
    public class MediaRepo : IMediaRepo
    {
        ...
        {
            var media = new Entities.Media() { Title = title, PublisedDateUtc = publisedDate, Description = description, LastConsumedDateUtc = lastConsumedDate, Rating = rating?.Value ?? -1 };
        ...
        public async Task<IEnumerable<Media>> GetAll()
        {
            ...
            return _mapper.Map<IEnumerable<Entities.Media>, IEnumerable<Media>>(mediaList);
        }
        ...
        public async Task<Media?> GetById(Guid id)
        {
            ...
            if (media == null)
                return null;
            return _mapper.Map<Entities.Media, Media>(media);
        }
        ...
        {
            return await _context.Medias.Where(m => m.Id == id).ExecuteUpdateAsync((m) => m.SetProperty(m => m.Rating, rating.Value)) == 1;
        }
